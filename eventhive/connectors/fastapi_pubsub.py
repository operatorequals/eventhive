...
class FastAPIPubSubConnector(base.BaseConnector):
    ...
    def __init__(self, connector_id, connector_config, global_config):
        ...
        logger.info("Initializing FastAPI PubSub Connector for '%s'" % connector_id)
    ...
    async def _subscriber_async(self, channels):
        self.SUBSCRIBER = PubSubClient(channels,
                callback=lambda data=None, topic=None :
                self.read_from_pubsub('{}' if data is None else data, channel=topic)
            )
    ...
    def subscribe(self, channels=[]):
        ...
        channels = [ "%s%s%s" % (self.conn_id, self.global_conf['channel_separator'], c) for c in channels ]
        ...
        self.SUBSCRIBER_THREAD = threading.Thread(target=self.SUBSCRIBER_LOOP.run_forever)
