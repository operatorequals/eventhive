...
class RedisConnector(base.BaseConnector):
    ...
    def __init__(self, connector_id, connector_config, global_config):
        ...
        self.REDIS = redis.Redis(**self.conn_conf['init'], decode_responses=True)
        logger.info("Redis Connector connected with: %s" % connector_config['init'])
    ...
    def subscribe(self):
        ...
        self.pusbsub_thread = self.PUBSUB.run_in_thread(exception_handler=exception_handler)
